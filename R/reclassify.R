reclassify <- function(varname,pcnt=NULL,adjust=TRUE,bestmod=TRUE,min.val=.1,diag=NULL,unif=NULL,dist=NULL,assoc=NULL,data=NULL) {
	...
	stopifnot(is.factor(data[,varname]))
	freq<-table(data[ ,varname])
	ncat<-length(freq)
